 ...
 * </copyright>
 */
package com.sudonlp.mining.word2vec;
...
import java.util.*;
import com.sudonlp.mining.word2vec.Vector;
...
public class DocVectorModel extends AbstractVectorModel<Integer>
{
     ...
     * @return 文档向量
     */
    public com.sudonlp.mining.word2vec.Vector addDocument(int id, String content)
    {
        com.sudonlp.mining.word2vec.Vector result = query(content);
     ...
     * @return 向量
     */
    public com.sudonlp.mining.word2vec.Vector query(String content)
    {
        if (content == null || content.length() == 0) return null;
        List<Term> termList = NotionalTokenizer.segment(content);
        com.sudonlp.mining.word2vec.Vector result = new com.sudonlp.mining.word2vec.Vector(dimension());
        int n = 0;
        for (Term term : termList)
        {
            com.sudonlp.mining.word2vec.Vector vector = wordVectorModel.vector(term.word);
            if (vector == null)
            {
                continue;
            }
    ...
    {
        com.sudonlp.mining.word2vec.Vector A = query(what);
        if (A == null) return -1f;
        Vector B = query(with);
        if (B == null) return -1f;
