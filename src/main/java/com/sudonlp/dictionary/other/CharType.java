...
public class CharType
{
    ...
    {
        type = new byte[65536];
        logger.info("字符类型对应表开始加载 " + SudoNLP.Config.CharTypePath);
        long start = System.currentTimeMillis();
        ByteArray byteArray = ByteArray.createByteArray(SudoNLP.Config.CharTypePath);
        if (byteArray == null)
        {
            try
            {
            ...
            {
                throw new IllegalArgumentException("字符类型对应表 " + SudoNLP.Config.CharTypePath + " 加载失败： " + TextUtility.exceptionToString(e));
            }
        }
...
//        System.out.print("int[" + typeList.size() + "][3] array = \n");
        DataOutputStream out = new DataOutputStream(new FileOutputStream(SudoNLP.Config.CharTypePath));
